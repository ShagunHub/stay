...
const Hero = () => {
  ...
  const onSearch= async(e)=>{
    ...
    // add destination to searchedCities (keep max 3 recent searched cities locally)
    setSearchedCities((prevSearchedCities)=>{
      const updatedSearchedCities = [destination, ...prevSearchedCities.filter(c=> c !== destination)];
      if(updatedSearchedCities.length > 3){
        updatedSearchedCities.length = 3; // keep most recent 3
      }
  ...
  return (
   ...
   <p className='bg-[#49B9FF]/50 px-3.5 py-1 rounded-full mt-20'>The Ultimate Experience</p>   
   ...
   <p className='max-w-130 mt-2 text-sm md:text-base'>Unparalled luxury and comfort await at the world's most exclusive hotels and resorts.Start you journey today.</p>
