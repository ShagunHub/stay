   ...
   const{user, navigate, isOwner, setShowHotelReg}=useAppContext();
    ...
    return (
            ...
            <nav className={`fixed top-0 left-0 w-full flex items-center justify-between px-4 md:px-16 lg:px-24 xl:px-32 transition-all duration-500 z-50 ${isScrolled ? "bg-white/80 shadow-md text-gray-700 backdrop-blur-lg py-3 md:py-4" : "py-4 md:py-6"}`}>
                ...
                {/* Logo */}
                <Link to="/" onClick={() => { window.scrollTo({ top: 0, behavior: 'smooth' }); setIsMenuOpen(false); }}>
                ...
                <div className="hidden md:flex items-center gap-4">
                 <img src={assets.searchIcon} alt="search" className={`${isScrolled ? 'invert' : ''} h-7`} />
                   ...
                   :
                   ( <button onClick={openSignIn} className={`bg-black text-white px-8 py-2.5 rounded-full transition-all duration-500 ${isScrolled ? 'bg-gray-800' : 'bg-black'}`}>
                        Login
                    </button>)
                   }
                </div>
                ...
                <div className={`fixed top-0 left-0 w-full h-screen bg-white text-base flex flex-col md:hidden items-center justify-center gap-6 font-medium text-gray-800 transition-all duration-500 ${isMenuOpen ? "translate-x-0" : "-translate-x-full"}`}>
                    ...
                    {navLinks.map((link, i) => (
                        <Link key={i} to={link.path} onClick={() => { setIsMenuOpen(false); window.scrollTo({ top: 0, behavior: 'smooth' }); }}>
                            {link.name}
                        </Link>
                    ))}
